...
$(document).ready(function(){
  ...
  return Number(numStr)
    ...
    $("#id_tipodeproducto").click(function() {
         $("#id_producto").load('{{url('comboca')}}' + '?id_tipodeproducto='  +this.options[this.selectedIndex].value) ;
       });
       
       
        $("#id_producto").click(function() {
       $("#detallep").load('{{url('detallep')}}' + '?id_producto=' + this.options[this.selectedIndex].value) ;
       });
...
<br>
Clave venta <input type = 'text'class="form-control form-control-user" name = 'idv' id= 'idv' value = '{{$idv}}' readonly = 'readonly'>
<br>
...
<br>
      ...
      <option value = '{{$cli->id_cliente}}'>{{$cli->nombre}} {{$cli->apellido1}} {{$cli->apellido2}}</option>
      @endforeach
...
<br>
 <select  class="form-control form-control-user" name = 'id_mesero' id= 'id_mesero'>
      ...
      <option value = '{{$emple->id_empleado}}'>{{$emple->nombree}} {{$emple->apellido1}} {{$emple->apellido2}}</option>
      @endforeach
...
<br>
    ...
    Seleccione Mesa<select  class="form-control form-control-user" name = 'id_mesa' id= 'id_mesa' >
      ...
      <option value = '{{$mes->id_mesa}}'>{{$mes->id_mesa}}</option>
      @endforeach
...
<select  class="form-control form-control-user" name = 'id_tipodeproducto' id= 'id_tipodeproducto'>
			...
			@foreach($tipodeproductos as $tp)
			<option value = '{{$tp->id_tipodeproducto}}'>{{$tp->nombre}}</option>
			@endforeach
